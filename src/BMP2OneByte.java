 ...
 * Rev: 2/21/09The array index values for passing arguments in main() were 1 and 2, should have been 0 and 1 (at least according to Conrad). Not tested.
 * Rev: 11/12/14Added the topDownDIB flag to deal with negative biHeight values which means image is stored rightside up. All loops depending on the
 ...
 * Rev: 01/10/17Was using the term 24-bit color when it was 32-bit in the comments. Fixed the documentation to be correct.
 * REV: 12/13/19Added a warning message if the bmpFileHeader_bfType is not the expected value of 0x4D42 which are the letters BM. This short field is no longer int-swapped.
 * Rev: 12/02/21Added a warning message if the bmpFileHeader_biSize is not the expected value of 40 which which means the file has structures we cannot interpret and so we skip over them
 ...
 *intrgbqToPel(int red, int green, int blue, int reserved)
 *RgbQuad pelToRGB(int pel)
 *intrgbToPel(int red, int green, int blue)
 *intcolorToGrayscale(int pel)
...
for (int num = 1; num <= 5 ; num ++) {// set <= 6
for (int set = 1; set <= 6; set++) {// num <= 5

inFileName = "Converted Images/f" + num + set + ".bmp";
outFileName = "Converted Bins/f" + num + set + ".bin";
...
*/
if (bmpFileHeader_bfType != 0x424D)
System.out.printf("WARNING! This image is NOT a BMP file! Processing anyway... Who am I to argue.\n\n");
...
ValueDescription
1The bitmap is monochrome, and the bmiColors member contains two entries.
...
2The bitmap has four possible color values.The most significant half-nibble is to the left in the image.
4The bitmap has a maximum of 16 colors, and the bmiColors member contains up to 16 entries.
...
8The bitmap has a maximum of 256 colors, and the bmiColors member contains up to 256 entries. In this case, each byte in the array represents a single pixel.
16The bitmap has a maximum of 2^16 colors.
...
24The bitmap has a maximum of 2^24 colors, and the bmiColors member is NULL.
...
BI_BITFIELDSSpecifies that the bitmap is not compressed and that the color table consists of three DWORD color masks that specify the red, green, and blue components of each pixel.
This is valid when used with 16- and 32-bpp bitmaps.
This value is valid in Windows Embedded CE versions 2.0 and later.
BI_ALPHABITFIELDSSpecifies that the bitmap is not compressed and that the color table consists of four DWORD color masks that specify the red, green, blue, and alpha components of each pixel.
This is valid when used with 16- and 32-bpp bitmaps.
...
{
iColumn = j * 2 + k;// 1 - kis needed to have High, Low nibble ordering for the image.
